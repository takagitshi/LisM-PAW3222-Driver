...
test:
	...
	OUTPUT_BIN="$$(mktemp -t paw3222-output-test.XXXXXX)"; \
	trap 'rm -f "$${DELTA_BIN}" "$${ACCEL_BIN}" "$${OUTPUT_BIN}"' EXIT; \
	...
	cc -std=c11 -Wall -Wextra -Werror -fsanitize=undefined \
		-Iinclude tests/paw3222_output_test.c -o "$${OUTPUT_BIN}"; \
	UBSAN_OPTIONS=halt_on_error=1 "$${OUTPUT_BIN}"
