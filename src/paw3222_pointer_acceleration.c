...
static void reset_axis(struct paw3222_pointer_accel_axis_state *axis) {
    axis->remainder = 0;
}
...
static int32_t scale_axis(struct paw3222_pointer_accel_axis_state *state, int32_t value,
                          uint32_t multiplier) {
    if (value == 0) {
        return 0;
    }
    ...
    const int64_t scaled = scaled_units / PAW3222_POINTER_ACCEL_GAIN_ONE;
    state->remainder =
        (int32_t)(scaled_units - scaled * PAW3222_POINTER_ACCEL_GAIN_ONE);
...
void paw3222_pointer_accel_apply_frame(const struct paw3222_pointer_accel_curve *curve,
                                       ...
                                       int32_t *out_y) {
    uint32_t speed = paw3222_pointer_accel_vector_speed(x, y);
    ...
    speed = paw3222_pointer_accel_normalize_speed(
        speed, curve->reference_interval_ms, collection_elapsed_ms);

    const uint32_t multiplier = paw3222_pointer_accel_multiplier(curve, speed);
